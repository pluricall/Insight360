...
export async function queryReportQueue(): Promise<QueueReportRow[]> {
  const result = await prisma.$queryRaw<QueueReportRow[]>`
      SELECT 
    ...
    SUM(CASE 
        ...
        AND (
            owner_name NOT LIKE '%Side%' 
        ) THEN 1 ELSE 0 
    END) AS sales,

    SUM(CASE WHEN servicos_assistencia LIKE '%Galp%' AND ( owner_name NOT LIKE '%Side%' ) THEN 1 ELSE 0 
